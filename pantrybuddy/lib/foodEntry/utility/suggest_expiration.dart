...
Future<String> suggestExpiration_Manual(String productName) async {
  try {
    ...
    // debugPrint("Loaded CSV Data: $expirationData");
    ...
    List<String> productNames = expirations
        .map((exp) => exp['NAME_all'].toString().toLowerCase())
        .toList();
    ...
    if (result.bestMatch.rating! > 0.8) {
      ...
      expiration = expirations.firstWhere(
          (exp) => exp['NAME_all'].toString().trim() == productName);
      ...
      if (expiration == null) {
        ...
        starting =
            'Based on similar expiration guidelines for ${expiration['Name_ALL']}, this item should be consumed within: \n';
      }
    }

    List<String> suggestions = [starting];
    if (expiration['DOP_Refrigerate_Max'] != '' &&
        expiration['DOP_Refrigerate_Metric'].toString().toLowerCase().trim() !=
            '') {
      suggestions.add(
          '\t${expiration['DOP_Refrigerate_Max']} ${expiration['DOP_Refrigerate_Metric']} if refrigerated from the date of purchase\n');
    }
    if (expiration['Refrigerate_After_Opening_Max'] != '' &&
        expiration['Refrigerate_After_Opening_Metric']
                .toString()
                .toLowerCase()
                .trim() !=
            '') {
      suggestions.add(
          '\t${expiration['Refrigerate_After_Opening_Max']} ${expiration['Refrigerate_After_Opening_Metric']} if refrigerated after opening\n');
    }
    if (expiration['Refrigerate_Max'] != '' &&
        expiration['Refrigerate_Metric'].toString().toLowerCase().trim() !=
            '') {
      suggestions.add(
          '\t${expiration['Refrigerate_Max']} ${expiration['Refrigerate_Metric']} if refrigerated from the date of purchase\n');
    }
    if (expiration['DOP_Pantry_Max'] != '' &&
        expiration['DOP_Pantry_Metric'].toString().toLowerCase().trim() != '') {
      suggestions.add(
          '\t${expiration['DOP_Pantry_Max']} ${expiration['DOP_Pantry_Metric']} if in the pantry from the date of purchase\n');
    }
    if (expiration['Pantry_Max'] != '' &&
        expiration['Pantry_Metric'].toString().toLowerCase().trim() != '') {
      suggestions.add(
          '\t${expiration['Pantry_Max']} ${expiration['Pantry_Metric']} if in the pantry from the date of purchase\n');
    }
...
// ignore: non_constant_identifier_names
Future<String> suggestExpiration_Auto(
    String productName, SearchType productType) async {
  try {
    // Load and parse the categories and expiration data from CSV files.
    // Use the correct asset path
    List<List<dynamic>> csvData = await loadCsv('assets/foodKeeper.csv');
    ...
    if (result.bestMatch.rating! > 0.4) {
      ...
      //matching based on tokenization
      ...
      for (var exp in expirations) {
        ...
        var matchCount =
            searchTokens.where((token) => productTokens.contains(token)).length;
        if (matchCount > 0) {
          // you might adjust this threshold based on needs
      ...
      if (filteredExpirations.isNotEmpty) {
        ...
        starting =
            'Based on similar expiration guidelines for ${expiration['Name'].toString().toLowerCase().trim()}, this item should be consumed within: \n';
      } else {
        if (productType == SearchType.menuItems) {
          String starting =
              'Based on the expiration guidelines for meals, this item should be consumed within:: \n';
          List<String> suggestions = [starting];
          suggestions
              .add('\t3-4 days if refrigerated from the date of purchase \n');
          suggestions
              .add('\t2-3 months if frozen from the date of purchase \n');
